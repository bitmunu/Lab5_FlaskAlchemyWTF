...
from new_forms import RegistrationForm, EditUserForm
from flask import Flask, request, render_template, redirect


app = Flask(__name__)

app.config['SECRET_KEY'] = 'pepega'
...
@app.route('/edit-user-profile/<int:user_id>', methods=["GET", "POST"])
def edit_user(user_id):
    user = session.get(User, user_id)
    if user is None:
        return f"Такого {user_id} не существует."
    ...
    if request.method == "GET":
        ...
        return render_template('edit-user-profile.html', form=form, user_id=user.id)
